...
const post = computed(() =>
  contentStore.blogPosts.find(
    (p) => String(p.id) === postId.value && p.status === "published"
  )
);
    ...
    <template v-else>
      ...
      >
        ...
        <div class="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-20">
          ...
          <nav class="flex items-center gap-2 text-sm text-muted mb-8">
            ...
            <NuxtLink to="/blog" class="hover:text-foreground transition-colors">{{
              t("nav.article")
            }}</NuxtLink>
            <UIcon name="i-lucide-chevron-right" class="size-4" />
          ...
          <!-- Title -->
          <h1
            class="text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tight mb-6 leading-tight"
          >
            {{ post.title }}
          </h1>
      ...
      <article class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-16">
        ...
        <div
          ...
          class="mb-10 rounded-2xl overflow-hidden border border-default shadow-lg"
        >
          <img
            v-if="postImages[0]"
        ...
        <div v-if="postImages.length > 1 || postVideos.length > 0" class="mt-12">
          ...
          <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            >
              <video :src="vid.url" controls class="w-full h-full object-contain" />
            </div>
          </div>
        </div>
