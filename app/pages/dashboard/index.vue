...
const nextInstructorDetails = computed(() => {
  ...
  // Instructors list is not loaded on this page; details are shown inline from the slot
  return null
})
...
const quickActions = computed(() => {
  const firstAction: { label: string; icon: string; to: string; color: 'warning' | 'neutral' | 'primary'; external?: boolean } = isCourseCompleted.value
              ...
              <div class="flex-1 space-y-4">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p class="font-medium">{{ nextSession.car || 'BYD Atto 1' }}</p>
                  </div>
                </div>
