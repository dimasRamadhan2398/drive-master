...
for DB in "${DATABASES[@]}"; do
  ...
  ssh -i deploy_key -o StrictHostKeyChecking=no -o IPQoS=none "$SSH_TARGET" "sudo -u postgres psql -c \"SELECT pg_terminate_backend(pg_stat_activity.pid) FROM pg_stat_activity WHERE pg_stat_activity.datname = '$DB' AND pid <> pg_backend_pid();\""
  ssh -i deploy_key -o StrictHostKeyChecking=no -o IPQoS=none "$SSH_TARGET" "sudo -u postgres psql -c \"DROP DATABASE IF EXISTS $DB;\""
  ssh -i deploy_key -o StrictHostKeyChecking=no -o IPQoS=none "$SSH_TARGET" "sudo -u postgres psql -c \"CREATE DATABASE $DB OWNER admin_drive;\""
  
  echo "Importing schema and data for $DB..."
  ssh -i deploy_key -o StrictHostKeyChecking=no -o IPQoS=none "$SSH_TARGET" "sudo -u postgres psql -d $DB -f $TARGET_DIR/database_dumps/$DB.sql"
done
